(function ($) {
  ...
  if (audio.length !== 0) {
    ...
    $('.js-waveform').each(function (index, element) {
      ...
      const durationId = `#waveform-duration-${index}`;
      ...
      document.querySelector(buttonId).addEventListener('click', function (e) {
        ...
        if (e.currentTarget.dataset.current === 'play') {
          e.currentTarget.dataset.current = 'pause';
          icon.className = 'fa fa-pause';
          $(buttonId).html('Pause ').append(icon);
        } else {
          e.currentTarget.dataset.current = 'play';
